...
  subgraph "Rank 1:R-1ㅤ"
    direction TB
    sp1[["`Sample proposals & \\ncompute weights`"]]
...
md"""
## 🌊 ARCHER2 weak scaling results
8 particles per rank, bootstrap proposals, 250 timesteps
"""

# ╔═╡ a3051e6a-2ca8-477f-ac8b-61b754e4a445
LocalResource("tsunami-weak-scaling-updated-$(selected_theme).svg", :width => 550)

# ╔═╡ 120a3751-1534-4a04-b18c-b9d933ced2ff
md"""
## 🌊 Kathleen weak scaling results

9600 particles per rank (240 particles / thread), bootstrap proposals, 50 time steps
"""

# ╔═╡ ad95e434-d219-48f7-9cdf-940b7773f9b8
let
	# kathleen_weak_scaling_run_times = Dict(
	# 	1 => 73.1,
	# 	2 => 126,
	# 	#3 => 80.5, 
	# 	#4 => 102,
	...
	# 	6 => 403,
	# )
	# kathleen_weak_scaling_efficiencies = Dict(
	# 	k => kathleen_weak_scaling_run_times[1] / v 
	# 	for (k, v) in pairs(kathleen_weak_scaling_run_times)
	# )
	# plot(
	# 	kathleen_weak_scaling_efficiencies,
...
# ╔═╡ 74807ed1-ca34-436e-8fcb-0cfa13663caf
md"""
## 🌊 Kathleen strong scaling results

9600 particles, bootstrap proposals, 50 timesteps
"""
...
# ╔═╡ c1d01ce1-cf86-4adf-82f0-3c18023291cc
let
	...
	kathleen_weak_scaling_efficiencies = Dict(
		k => kathleen_weak_scaling_run_times[2] / v 
		for (k, v) in pairs(kathleen_weak_scaling_run_times)
	)
	plot(
		kathleen_weak_scaling_efficiencies,
	...
	savefig("kathleen-weak-scaling.svg")
end

# ╔═╡ 1ba0bd25-f130-416f-abc5-50fc32201c7b
let
	set_plot_style(selected_theme)
	kathleen_strong_scaling_run_times = Dict(
		#1 => 73.1,
		2 => 29.8,
		3 => 41.5,
		4 => 43.8,
	...
	kathleen_strong_scaling_efficiencies = Dict(
		k => (kathleen_strong_scaling_run_times[2] * 2)/ (v * k) 
		for (k, v) in pairs(kathleen_strong_scaling_run_times)
	)
	plot(
		kathleen_strong_scaling_efficiencies,
		xlabel="Number of ranks",
...
# ╔═╡ b36743dd-96ed-4274-bb4a-c425a3c67342
let
	kathleen_strong_scaling_run_times = Dict(
		2 => (29.8, 21.6),
		3 => (41.5, 28.5),
